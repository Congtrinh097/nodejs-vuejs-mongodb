...
<script>
  ...
  export default {
    ...
    methods: {
      doLogin() {
        this.$store.dispatch('session/login', {email: this.email, password: this.password, rememberMe: this.rememberMe})
          .then((result) => {
            console.log(11111, result);
            if(result.status === 'success') {
              this.$router.push({ name: 'home' }, null, () => {});
            } else {
              displayNotification(this, { title: "Login error", message: result.message, variant: 'warning' });
            }
        })
